...
export function RegionRanking({
  ...
  return (
    <div className="overflow-hidden rounded-[24px] border border-[var(--border)]">
      <div className="grid grid-cols-[minmax(0,1.2fr)_110px_90px_90px] gap-3 border-b border-[var(--border)] bg-[rgba(33,150,243,0.05)] px-4 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--muted)]">
      ...
      <ol className="divide-y divide-[var(--border)]">
        {items.slice(0, 12).map((item) => (
          <li
            ...
            className="grid grid-cols-[minmax(0,1.2fr)_110px_90px_90px] gap-3 px-4 py-3 text-sm text-[var(--foreground)]"
          >
            <div className="min-w-0">
              <p className="truncate font-medium">{item.name}</p>
            </div>
            <span className="text-[var(--muted)]">{item.country}</span>
            <span className="text-right font-medium">{formatInteger(item.companyCount, locale)}</span>
            <span className="text-right text-[var(--muted)]">{formatPercent(item.shareOfScope, locale)}</span>
          </li>
        ))}
      </ol>
    </div>
