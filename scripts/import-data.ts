...
const DASHBOARD_STANDARD_COLUMNS_NORMALIZED = [
  "nombre",
  "webpage",
  "nif",
  "bvd code",
...
const EXPECTED_ATTRIBUTE_COLUMNS_NORMALIZED = EXPECTED_ATTRIBUTE_COLUMNS.map((label) =>
  normalizeHeader(label).toLowerCase(),
);
const FALSE_ATTRIBUTE_SYMBOL = String.fromCodePoint(0x2bbd);
const TRUE_ATTRIBUTE_SYMBOLS = new Set([String.fromCodePoint(0x2713), String.fromCodePoint(0x2714), String.fromCodePoint(0x2611), String.fromCodePoint(0x2705)]);
const FALSE_ATTRIBUTE_SYMBOLS = new Set([FALSE_ATTRIBUTE_SYMBOL]);
...
function createCompanyRecord(dashboardRow: DashboardCompanyRow, sabiRow: SabiRow | undefined): CompanyUpsert {
  return {
    ...
    latest_period_end: toNullableDate(getSabiValue(sabiRow, "latest_period_end")),
